...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.acknum = 0
        self.queue = []
        self.pkt_in_flight = False
        self.seqnum_limit = seqnum_limit
        self.base = 0
        self.to_send = 0
        self.window_size = seqnum_limit // 2
        self.timeout_value = 20 + 4 * self.window_size
        pass
    ...
    def output(self, message):
        ...
        if self.next_seqnum > self.seqnum_limit - 1:
            self.next_seqnum = 0
        checksum = self.next_seqnum + self.acknum + sum(message.data)
        pkt = Pkt(self.next_seqnum, 0, checksum, message.data)
        if TRACE > 0:
            print(b"Packet just added to queue with payload: " + pkt.payload + b' seq: ' + str(pkt.seqnum).encode() + b' ack: ' + str(pkt.acknum).encode())
        self.queue.append(pkt)
        self.sendIfPossible()
        

    def sendIfPossible(self):
        while self.to_send < self.base + self.window_size and self.to_send < len(self.queue):
            pkt = self.queue[self.to_send]
            if TRACE > 0:
                print(b"Just sent packet to B, packet: " + pkt.payload + b' ' + str(pkt.seqnum).encode())
            to_layer3(self,pkt)
            self.to_send += 1
            if TRACE > 0:
                print("Current base: " + str(self.base) + " New to_send: " + str(self.to_send))
            #stop_timer(self)
            start_timer(self, self.timeout_value)

        # if self.pkt_in_flight == False and len(self.queue) > 0:
        #     pkt = self.queue[0]
        #     self.pkt_in_flight = True
        #     if TRACE > 0:
        #         print(b"Just sent packet to B, packet: " + pkt.payload + b' ' + str(pkt.seqnum).encode())
        #     to_layer3(self, pkt)
        #     start_timer(self, 15)

    def findIndexInQueueOfPacket(self, packet):
        for x in range(self.base, self.to_send):
            proposedPacket = self.queue[x]
            if proposedPacket.seqnum == packet.acknum and proposedPacket.payload == packet.payload:
                if TRACE > 0:
                    print("FindIndex for packet with ACK: " + str(packet.acknum) + " and payload: " + str(packet.payload) + " Index Found: " + str(x))
                return x
        if TRACE > 0:
            print("Find index didn't work for some reason.")
        return -1
    ...
    def input(self, packet):
        indexOfPacketInQueue = self.findIndexInQueueOfPacket(packet)

        if indexOfPacketInQueue == -1:
            if TRACE > 0:
                print("FindIndex couldn't find the right index. Possible that the packet was already ACKed. Returning...")
            return
        
        pkt = self.queue[indexOfPacketInQueue]
        expectedChecksum = packet.seqnum + packet.acknum + sum(packet.payload)
        if TRACE > 0:
            print(b"Received packet on A side: " + packet.payload + b' seq: ' + str(packet.seqnum).encode() + b' ack: ' + str(packet.acknum).encode() + b' incoming checksum: '
                  + str(packet.checksum).encode() + b' computed checksum: ' + str(expectedChecksum).encode() + b' checksum of queue prev: ' + str(pkt.checksum).encode())
        if pkt.seqnum == packet.acknum and expectedChecksum == packet.checksum and pkt.seqnum == packet.seqnum:
            if TRACE > 0:
                print(b"Packet received is correct, moving onto sending next")
            self.base = indexOfPacketInQueue + 1
            if TRACE > 0:
                print("New base: " + str(self.base) + " and current to_send: " + str(self.to_send))
            if self.base == self.to_send:
                stop_timer(self)
            else:
                stop_timer(self)
                start_timer(self, self.timeout_value)
            self.sendIfPossible()
        elif expectedChecksum != packet.checksum:
            if TRACE > 0:
                print(b"Packet received is corrupted, returning...")
            return
        elif pkt.seqnum != packet.acknum:
            if TRACE > 0:
                print(b"Duplicate ACK came back, returning...")
            return
        ...
        # else:
        #     if TRACE > 0:
        #         print("The improbable has happened. Resend packet")
        #     self.sendIfPossible()
    ...
    def timer_interrupt(self):
        if TRACE > 0:
                print(b"Timer went off, resending packets")
        stop_timer(self)
        start_timer(self, self.timeout_value)
        for x in range(self.base, self.to_send):
            pkt = self.queue[x]
            if TRACE > 0:
                print(b"Resending packets to B, packet: " + pkt.payload + b' ' + str(pkt.seqnum).encode())
            to_layer3(self, pkt)
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.acknum = 0
        self.last_successful_packet = None
        self.seqnum_limit = seqnum_limit
        pass
    ...
    def input(self, packet):
        if TRACE > 0:
                print(b"Received packet on B side: " + packet.payload + b' ' + str(packet.seqnum).encode())
        expectedChecksum = packet.checksum
        seqNum = packet.seqnum
        actualChecksum = packet.seqnum + packet.acknum + sum(packet.payload)
        if actualChecksum != expectedChecksum:
            if TRACE > 0:
                print(b"Packet on B side doesn't match checksum, sending back most recent successful packet")
            # return
            to_layer3(self, self.last_successful_packet)
        elif seqNum != self.expectedSeqNum:
            if TRACE > 0:
                print(b"Packet on B side has unexpected seqNum, sending back most recent successful packet")
            # return
            to_layer3(self, self.last_successful_packet)
        else:
            newChecksum = seqNum + seqNum + sum(packet.payload)
            pkt = Pkt(seqNum, seqNum, newChecksum, packet.payload)
            if TRACE > 0:
                print(b"Packet on B side looks good, sending ACK and up: " + b' payload: ' + pkt.payload + b' seq: ' + str(pkt.seqnum).encode() + b' ack: ' + str(pkt.seqnum).encode())
            self.expectedSeqNum = self.expectedSeqNum + 1
            if self.expectedSeqNum > self.seqnum_limit - 1:
                self.expectedSeqNum = 0
            self.last_successful_packet = pkt
